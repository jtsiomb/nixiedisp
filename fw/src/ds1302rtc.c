...
#define CLOCK(x) \
	do { if(x) PORTB |= PB_RTC_CK; else PORTB &= ~PB_RTC_CK; } while(0)
#define CE(x) \
	do { if(x) PORTC |= PC_RTC_EN; else PORTC &= ~PC_RTC_EN; } while(0)
#define SET_IO(x) \
	do { if(x) PORTB |= PB_RTC_DATA; else PORTB &= ~PB_RTC_DATA; } while(0)
#define GET_IO ((PINB & PB_RTC_DATA) ? 1 : 0)
#define DDR_IO(x) \
	do { if(x) DDRB |= PB_RTC_DATA; else DDRB &= ~PB_RTC_DATA; } while(0)
...
{
	tm->sec = read_reg(REG_SEC);
	tm->min = read_reg(REG_MIN);
	tm->hour = read_reg(REG_HOUR);
}
